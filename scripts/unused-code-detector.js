...
function checkUnusedExports() {
  ...
  try {
    execSync(
      'npx ts-unused-exports tsconfig.json --ignoreFiles=".*\\.test\\.tsx?$"',
      {
        stdio: 'inherit',
        encoding: 'utf8',
      }
    ...
    log(
      '⚠️  ts-unused-exports not available. Install with: npm install -g ts-unused-exports',
      'yellow'
    );
  }
}
...
function checkUnusedDependencies() {
  ...
  try {
    ...
    if (result.dependencies?.length > 0) {
      log(
        `\n❌ Unused dependencies found (${result.dependencies.length}):`,
        'red'
      );
      ...
      log(
        '\nRemove with: npm uninstall ' + result.dependencies.join(' '),
        'yellow'
      );
    } else {
    ...
    if (result.devDependencies?.length > 0) {
      log(
        `\n⚠️  Unused devDependencies found (${result.devDependencies.length}):`,
        'yellow'
      );
    ...
    if (result.missing && Object.keys(result.missing).length > 0) {
      ...
      Object.keys(result.missing).forEach((dep) => {
        log(
          `   - ${dep} (used in ${result.missing[dep].length} file(s))`,
          'red'
        );
      });
      log(
        '\nInstall with: npm install ' + Object.keys(result.missing).join(' '),
        'yellow'
      );
    }
  } catch (error) {
    log(
      "Error running depcheck. Make sure it's installed: npm install -g depcheck",
      'red'
    );
  }
}
...
function findCommentedCode() {
  ...
  try {
    ...
    const result = execSync('grep -r "^[[:space:]]*//" src/ pages/ | wc -l', {
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'ignore'],
    }).trim();
