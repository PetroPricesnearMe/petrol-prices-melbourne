...
if (major === 5 && minor === 3) {
  console.log(
    '   ✓ TypeScript 5.3.3 - Fully supported by @typescript-eslint v6.x'
  );
} else if (major === 5 && minor < 4) {
  console.log(
    `   ⚠️  TypeScript ${major}.${minor}.x - Compatible but not optimal`
  );
} else if (major === 5 && minor >= 4) {
  console.log(
    `   ❌ TypeScript ${major}.${minor}.x - NOT supported by @typescript-eslint v6.x`
  );
  console.log('      Please downgrade to TypeScript 5.3.3');
  process.exit(1);
} else {
  console.log(
    `   ⚠️  TypeScript ${major}.${minor}.x - May have compatibility issues`
  );
}
...
if (eslintPluginVersion === eslintParserVersion) {
  ...
  console.log(
    '   ⚠️  @typescript-eslint versions mismatch - should be identical'
  );
}
...
if (eslintMajor === 6) {
  console.log(
    '   ✓ @typescript-eslint v6.x - Supports TypeScript 4.3.5 - 5.3.x'
  );
} else if (eslintMajor === 7) {
  console.log(
    '   ⚠️  @typescript-eslint v7.x - Requires TypeScript 4.7.4 - 5.4.x'
  );
  console.log('      Consider downgrading to v6.21.0 for TypeScript 5.3.3');
} else if (eslintMajor < 6) {
  console.log(
    '   ⚠️  @typescript-eslint v' +
      eslintMajor +
      '.x - Outdated, please upgrade'
  );
}
...
try {
  ...
  if (!foundUnsupported) {
    ...
    console.log(
      '   ⚠️  Found TypeScript 5.4+ features - may need refactoring\n'
    );
  }
} catch (err) {
