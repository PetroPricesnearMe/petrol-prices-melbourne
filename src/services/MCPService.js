...
class MCPService {
  ...
  connect() {
    ...
    try {
      console.warn('🔌 [MCP] Connecting to SSE stream...');
      console.warn(`📍 [MCP] URL: ${this.sseUrl}`);

      this.eventSource = new EventSource(this.sseUrl);

      // Connection opened
      this.eventSource.onopen = () => {
        console.warn('✅ [MCP] Connected to real-time updates');
      ...
      this.eventSource.onmessage = (event) => {
        try {
          ...
          console.warn('📨 [MCP] Received update:', data);
  ...
  setupBaserowEventListeners() {
    ...
    this.eventSource.addEventListener('row.created', (event) => {
      const data = JSON.parse(event.data);
      console.warn('➕ [MCP] Row created:', data);
    ...
    this.eventSource.addEventListener('row.updated', (event) => {
      const data = JSON.parse(event.data);
      console.warn('🔄 [MCP] Row updated:', data);
    ...
    this.eventSource.addEventListener('row.deleted', (event) => {
      const data = JSON.parse(event.data);
      console.warn('➖ [MCP] Row deleted:', data);
    ...
    this.eventSource.addEventListener('table.updated', (event) => {
      const data = JSON.parse(event.data);
      console.warn('🗃️ [MCP] Table updated:', data);
   ...
   */
  disconnect() {
    console.warn('🔌 [MCP] Disconnecting...');
   ...
   */
  handleUpdate(data) {
    const { table_id, row_id, payload } = data;
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('load', () => {
    console.warn('🚀 [MCP] Auto-connecting to real-time updates...');
    mcpService.connect();
  });
