...
const config = {
  ...
  google: {
    placesApiKey:
      ...
      'AIzaSyDfEKO1GZBpuUuhhL-gz1miug6jdlT1nFk',
  ...
  app: {
    ...
    description:
      process.env.REACT_APP_APP_DESCRIPTION || 'Melbourne Petrol Stations',
  },
};
...
export const baserowAPI = {
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn(
              `⚠️ Rate limited (429). Waiting ${waitTime / 1000}s before retry...`
            );
          }
        ...
        if (response.ok) {
          if (process.env.NODE_ENV === 'development') {
            console.log(
              `✅ Request successful after ${attempt + 1} attempt(s)`
            );
          }
          return response;
        }
        ...
        if (process.env.NODE_ENV === 'development') {
          if (error.name === 'AbortError') {
            console.warn(
              `⚠️ Request timeout on attempt ${attempt + 1}/${maxRetries}`
            );
          } else {
            console.warn(
              `⚠️ Request failed on attempt ${attempt + 1}/${maxRetries}: ${error.message}`
            );
          }
        }
  ...
  async fetchAllStations() {
    ...
    if (!config.api.baseUrl || config.api.baseUrl === '/api') {
      ...
      try {
        return await this.fetchAllStationsDirect(
          config.tables.petrolStations.id
        );
      } catch (error) {
        if (process.env.NODE_ENV === 'development') {
    ...
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `🔄 Fetching all stations from: ${config.api.baseUrl}/api/stations/all`
        );
      }

      // Use retry logic for backend API calls
      const response = await this.fetchWithRetry(
        `${config.api.baseUrl}/api/stations/all`,
        {
          method: 'GET',
          headers: {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ Successfully fetched ${data.data.length} stations from backend`
        );
      }
      return data.data;
    } catch (error) {
      ...
      if (
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '🔄 Backend unavailable, trying direct Baserow API as fallback...'
          );
        }
        try {
          return await this.fetchAllStationsDirect(
  ...
  async createStation(stationData) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ Successfully created station: ${stationData.stationName}`
        );
      }
      return data.data;
    } catch (error) {
  ...
  async updateStation(stationId, updateData) {
    try {
      ...
      const response = await fetch(
        `${config.api.baseUrl}/api/stations/${stationId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  async getTableFields(tableId) {
    try {
      const response = await fetch(
        `${config.api.baseUrl}/api/baserow/fields/${tableId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async fetchAllStationsDirect(tableId) {
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        '⚠️ Using direct Baserow API access. Consider using backend proxy instead.'
      );
    }
    ...
    const usePublicToken =
      config.baserow.publicToken &&
      config.baserow.publicToken !== 'your_public_token_here';
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(
        `🔄 Fetching directly from Baserow API: ${nextUrl.replace(config.baserow.publicToken, 'PUBLIC_TOKEN')}`
      );
      console.log(`📊 Database ID: ${config.baserow.databaseId}`);
      console.log(
        `🔑 Using ${usePublicToken ? 'public token' : 'auth token'}: ${(usePublicToken ? config.baserow.publicToken : config.baserow.token).substring(0, 8)}...`
      );
    }
    ...
    try {
      while (nextUrl) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            `📡 Making request to: ${nextUrl.replace(config.baserow.publicToken, 'PUBLIC_TOKEN')}`
          );
        }
        ...
        nextUrl = data.next
          ? usePublicToken
            ? `${data.next}&public_token=${config.baserow.publicToken}`
            : data.next
          : null;
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `✅ Successfully fetched ${rows.length} stations from Baserow`
        );
      }
      return rows;
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
        console.error(
          '❌ Error fetching stations from Baserow:',
          error.message
        );
      }
      throw error;
