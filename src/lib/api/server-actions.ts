...
export const getStationById = cache(
  async (id: number): Promise<Station | null> => {
    // Validate input
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch station ${id}: ${response.statusText}`
        );
      }
...
export const getFuelPrices = cache(async (): Promise<FuelPrice[]> => {
  try {
    const response = await fetch(
      `${process.env.BASEROW_API_URL}/api/database/rows/table/${process.env.BASEROW_PRICES_TABLE_ID}/`,
      {
        headers: {
          Authorization: `Token ${process.env.BASEROW_API_TOKEN}`,
          'Content-Type': 'application/json',
    ...
    case 'price-low':
      return [...stations].sort((a, b) => {
        const aPrice = Math.min(
          ...(a.fuelPrices?.map((fp) => fp.price) || [Infinity])
        );
        const bPrice = Math.min(
          ...(b.fuelPrices?.map((fp) => fp.price) || [Infinity])
        );
