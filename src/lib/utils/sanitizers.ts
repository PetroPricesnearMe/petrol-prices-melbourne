...
export function sanitizeObject<T extends Record<string, unknown>>(
  ...
  for (const key in sanitized) {
    ...
    if (typeof value === 'string') {
      sanitized[key] = sanitizeString(value, maxLength) as T[Extract<keyof T, string>];
    } else if (value && typeof value === 'object' && !Array.isArray(value)) {
      sanitized[key] = sanitizeObject(value as Record<string, unknown>, maxLength) as T[Extract<keyof T, string>];
...
export function validateAndSanitizeStation(station: unknown): {
  ...
  if (stationObj.prices && typeof stationObj.prices === 'object' && !Array.isArray(stationObj.prices)) {
    sanitized.prices = {};
    const prices = stationObj.prices as Record<string, unknown>;
    
    Object.keys(prices).forEach((fuelType) => {
      const price = typeof prices[fuelType] === 'string' 
