...
import { baserowServerService as baserowService } from '../services/BaserowServerService';
...
export async function loadStationsFromBaserow() {
  if (!baserowService) {
    console.warn(
      '⚠️ [Server] Baserow service not available, falling back to GeoJSON'
    );
    return await loadStationsFromGeoJSON();
  }

  try {
    console.log('🔄 [Server] Loading stations from Baserow...');
    const stations = await baserowService.fetchStationsWithPrices();

    if (!stations || stations.length === 0) {
      console.warn(
        '⚠️ [Server] No stations from Baserow, falling back to GeoJSON'
      );
      return await loadStationsFromGeoJSON();
    }

    console.log(`✅ [Server] Loaded ${stations.length} stations from Baserow`);
    return stations;
  } catch (error) {
    console.error(
      '❌ [Server] Error loading from Baserow, falling back to GeoJSON:',
      error.message
    );
    return await loadStationsFromGeoJSON();
  }
