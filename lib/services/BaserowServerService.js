...
class BaserowServerService {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (response.status === 429) {
          ...
          console.warn(`⚠️ [Baserow] Rate limited. Waiting ${waitTime / 1000}s...`);
        ...
        if (attempt === maxRetries - 1) {
          console.error(`❌ [Baserow] Request failed after ${maxRetries} attempts:`, error.message);
          throw lastError;
        }

        const backoffTime = Math.pow(2, attempt) * 1000;
        console.log(`⏳ [Baserow] Retrying in ${backoffTime / 1000}s... (attempt ${attempt + 1}/${maxRetries})`);
  ...
  async fetchAllRows(tableId, usePublicToken = true) {
    ...
    try {
      while (nextUrl) {
        console.log(`📡 [Baserow] Fetching page ${page} from table ${tableId}...`);
        ...
        console.log(`✓ Page ${page}: ${data.results.length} rows (total: ${allRows.length})`);
      ...
      console.log(`✅ [Baserow] Fetched ${allRows.length} total rows from table ${tableId}`);
      return allRows;
      
    } catch (error) {
      console.error(`❌ [Baserow] Error fetching rows from table ${tableId}:`, error.message);
      throw error;
    }
  }
  ...
  async fetchStationsWithPrices() {
    try {
      ...
      console.log(`✅ [Baserow] Merged ${stationsWithPrices.length} stations with prices`);
      return stationsWithPrices;
      
    } catch (error) {
      console.error('❌ [Baserow] Error fetching stations with prices:', error.message);
      throw error;
    }
  }
